...
export default function MarketingPage() {
  ...
  const roiColor = (roi: number) =>
    roi >= 4 ? 'text-success' : roi >= 3 ? 'text-info' : 'text-warning';
    ...
    <DashboardShell active="Marketing">
      <div className="space-y-5">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-lg font-semibold text-hi">Marketing</h1>
            <p className="text-xs text-lo mt-0.5">Campaign performance and ROI tracking</p>
          </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
              <div className="bg-surface border border-rim rounded-lg p-5">
                <div className="text-xs font-medium text-lo uppercase tracking-wider">Total Spend</div>
                <div className="mt-2 text-2xl font-semibold text-hi">${summary.totalSpend.toLocaleString()}</div>
              </div>
              <div className="bg-surface border border-success/30 rounded-lg p-5 bg-success/5">
                <div className="text-xs font-medium text-success uppercase tracking-wider">ROI</div>
                <div className="mt-2 text-2xl font-semibold text-success">{summary.roi}x</div>
              </div>
              <div className="bg-surface border border-rim rounded-lg p-5">
                <div className="text-xs font-medium text-lo uppercase tracking-wider">Impressions</div>
                <div className="mt-2 text-2xl font-semibold text-hi">{summary.impressions.toLocaleString()}</div>
              </div>
              <div className="bg-surface border border-rim rounded-lg p-5">
                <div className="text-xs font-medium text-lo uppercase tracking-wider">Conversions</div>
                <div className="mt-2 text-2xl font-semibold text-hi">{summary.conversions}</div>
              </div>
            </div>
            ...
            {showForm && (
              <form onSubmit={handleCreate} className="bg-surface border border-rim rounded-lg p-5 space-y-4">
                <div className="text-sm font-medium text-hi">New Campaign</div>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <div>
                    <label className="block text-xs text-mid mb-1.5">Channel</label>
                    <select
                      value={form.channel}
                  ...
                  <div>
                    <label className="block text-xs text-mid mb-1.5">Budget</label>
                    <input
                      type="number"
                      step="0.01"
                      value={form.spend}
                  ...
                  <div>
                    <label className="block text-xs text-mid mb-1.5">Status</label>
                    <select
                      value={form.status}
            ...
            <div className="bg-surface border border-rim rounded-lg overflow-hidden">
              <div className="border-b border-rim px-5 py-3.5 flex items-center justify-between">
                <span className="text-sm font-medium text-hi">Campaigns</span>
                <span className="text-xs text-lo">{campaigns.length} total</span>
              </div>
              {campaigns.length === 0 ? (
                <EmptyState
                    ...
                    <tbody>
                      {campaigns.map((c) => (
                        <tr key={c.id} className="border-b border-rim/50 hover:bg-raised/40 transition-colors">
                          <td className="px-5 py-3 text-mid font-medium">{c.name}</td>
                          ...
                          <td className="px-5 py-3 text-lo">${(c.spend || 0).toLocaleString()}</td>
                          <td className="px-5 py-3 text-lo">{c.conversions || 0}</td>
