    ...
    <DashboardShell active="Customers">
      <div className="space-y-5">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-lg font-semibold text-hi">Customers</h1>
            <p className="text-xs text-lo mt-0.5">CRM, segments, and lifetime value tracking</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <div className="bg-surface border border-rim rounded-lg p-5">
            <div className="text-xs font-medium text-lo uppercase tracking-wider">Total Customers</div>
            <div className="mt-2 text-2xl font-semibold text-hi">{totalCustomers.toLocaleString()}</div>
          </div>
          <div className="bg-surface border border-rim rounded-lg p-5">
            <div className="text-xs font-medium text-lo uppercase tracking-wider">VIP Members</div>
            <div className="mt-2 text-2xl font-semibold text-warning">{vipCount}</div>
          </div>
          <div className="bg-surface border border-rim rounded-lg p-5">
            <div className="text-xs font-medium text-lo uppercase tracking-wider">Total Lifetime Value</div>
            <div className="mt-2 text-2xl font-semibold text-success">${totalValue.toLocaleString()}</div>
          </div>
          <div className="bg-surface border border-rim rounded-lg p-5">
            <div className="text-xs font-medium text-lo uppercase tracking-wider">Avg Lifetime Value</div>
            <div className="mt-2 text-2xl font-semibold text-hi">${avgValue}</div>
          </div>
        </div>
        ...
        {showForm && (
          <form onSubmit={handleCreate} className="bg-surface border border-rim rounded-lg p-5 space-y-4">
            <div className="text-sm font-medium text-hi">New Customer</div>
        ...
        <div className="bg-surface border border-rim rounded-lg overflow-hidden">
          <div className="border-b border-rim px-5 py-3.5 flex items-center justify-between">
            <span className="text-sm font-medium text-hi">All Customers ({filtered.length})</span>
            <div className="relative">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-lo" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <input
                ...
                placeholder="Search..."
                className="bg-inset border border-edge rounded-lg pl-8 pr-3 py-1.5 text-xs text-hi placeholder-lo focus:outline-none focus:border-accent w-44 transition-colors"
              />
            </div>
          </div>

          {loading ? (
            <div className="p-8 text-center text-lo text-sm">Loading customers...</div>
          ) : filtered.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {paged.map((c) => (
                    <tr key={c.id} className="border-b border-rim/50 hover:bg-raised/40 transition-colors">
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          <Avatar name={`${c.firstName} ${c.lastName}`} size="sm" />
                          <span className="text-mid font-medium">{c.firstName} {c.lastName}</span>
                        </div>
                      </td>
                      <td className="px-5 py-3 text-lo">{c.email || '—'}</td>
                      <td className="px-5 py-3 text-lo">{c.phone || '—'}</td>
                      <td className="px-5 py-3 text-success font-medium">
