...
export default function MetricCard({
  ...
  return (
    <div className="relative overflow-hidden rounded-lg border border-[#2a2a3a] bg-[#12121a] p-5">
      <div className={`absolute inset-0 ${cfg.glow}`} />
      <div className="relative">
        <div className="text-xs font-medium text-[#6a6a80] uppercase tracking-wider">{title}</div>
        <div className={`mt-2 text-2xl font-semibold tabular-nums ${cfg.text}`}>{value}</div>
        {(helper || trend) && (
          <div className="mt-2 flex items-center gap-2">
            {helper && <span className="text-xs text-[#6a6a80]">{helper}</span>}
            {trend && (
