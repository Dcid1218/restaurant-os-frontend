...
export default function DashboardShell({
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-60 flex flex-col border-r border-slate-700 bg-slate-900 transition-transform duration-200 lg:translate-x-0 ${
      ...
      >
        ...
        <div className="h-14 flex items-center justify-between px-5 border-b border-slate-700 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold tracking-tight text-white">RestaurantOS</span>
          </div>
          <button
            onClick={() => setSidebarOpen(false)}
            className="lg:hidden w-6 h-6 flex items-center justify-center text-slate-500 hover:text-white transition-colors"
          >
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-0.5">
          {nav.map((item) => {
            ...
            return (
              ...
              >
                <span className={isActive ? 'text-indigo-500' : 'text-slate-500'}>{item.icon}</span>
      ...
      <main className="lg:ml-60">
        {/* Top header */}
        <header className="sticky top-0 z-30 h-14 border-b border-slate-700 bg-slate-900/80 backdrop-blur-md flex items-center justify-between px-4 lg:px-6">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setSidebarOpen(true)}
              className="lg:hidden w-8 h-8 flex items-center justify-center rounded-md text-slate-500 hover:text-white hover:bg-slate-700 transition-colors"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-md bg-slate-700 border border-slate-700">
              <svg className="w-3.5 h-3.5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <circle cx="11" cy="11" r="8" />
                <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-4.35-4.35" />
              </svg>
              <span className="text-xs text-slate-500">Search...</span>
              <kbd className="text-xs text-slate-500 bg-slate-800 px-1 rounded">⌘K</kbd>
            </div>
            <Avatar name={userName} size="sm" />
          </div>
        </header>
